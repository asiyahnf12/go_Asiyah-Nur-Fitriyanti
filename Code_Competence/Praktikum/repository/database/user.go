...
func GetUsers() (users []model.User, err error) {
	if err = config.DB.Model(&model.User{}).Preload("Items").Find(&users).Error; err != nil {
		return
	}
	return
}
...
func GetUserWithItem(id uint) (user model.User, err error) {
	user.ID = id
	if err = config.DB.Model(&model.User{}).Preload("Items").First(&user).Error; err != nil {
		return
	}
	return
}
